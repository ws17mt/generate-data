...
rm ${MODEL_DIR}/fren.${BPE_OPERATIONS}.bpe

echo "*** Learning BPE model"
# BPE (train)
cat ${PROC_DATA_DIR}/mono.tc.fr ${PROC_DATA_DIR}/mono.tc.en \
  | ${TOOLS_DIR}/learn_bpe.py -s ${BPE_OPERATIONS} > ${MODEL_DIR}/fren.${BPE_OPERATIONS}.bpe

for l in fr en; do
  echo "*** Applying BPE to mono.${l}"
  # BPE (apply)
  ${TOOLS_DIR}/apply_bpe.py -c ${MODEL_DIR}/fren.${BPE_OPERATIONS}.bpe \
    < ${PROC_DATA_DIR}/mono.tc.${l} \
